...
func getBetterHandType(hand string) string {
    cards := []rune{'A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2'}

    if strings.ContainsRune(hand, 'J'){
        betterValue := handTypes[getType(hand)]
        betterType := getType(hand)
        for _, card := range(cards){
            temp := strings.ReplaceAll(hand, "J", string(card))
            typ := getType(temp)
            if handTypes[typ] > betterValue {
                betterValue = handTypes[typ]
                betterType = typ
            }
        }
        return betterType
...
func orderHands(hands []inputType){
    ...
    for i := n-1; i > 0; i-- {
        for b := 0; b < i; b++ {
            ...
            if strings.Compare(hands[b].bestHandType, hands[b+1].bestHandType) == 0{
                ...
                if handTypes[hands[b].bestHandType] > handTypes[hands[b+1].bestHandType] {
                    temp := hands[b]
                    hands[b] = hands[b+1]
                    hands[b+1] = temp
                }
...
func main(){
    ...
    for scan.Scan(){
        ...
        it.bestHandType = getBetterHandType(it.hand)
